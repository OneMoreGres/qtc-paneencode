...
# modify sources
modify_data = ''
modify_file = 'qtcreator/src/plugins/projectexplorer/buildmanager.h'
with open(modify_file, 'r') as f:
    modify_data = f.read()

if modify_data.find('QtcPaneEncodePlugin') == -1:
    modify_data = modify_data.replace(
        '''namespace ProjectExplorer {''',
        '''namespace QtcPaneEncode { namespace Internal { class QtcPaneEncodePlugin; } }
namespace ProjectExplorer {''')
...
{''', '''class PROJECTEXPLORER_EXPORT BuildManager : public QObject
{
    friend class QtcPaneEncode::Internal::QtcPaneEncodePlugin;''')
